...
const Button = ({icon, children, href, containerClassName, onClick, markerFill}) => {
  const Inner = () => (
    <><span className="relative flex items-center m-h-[60px] px-4 g4 rounded-2xl inner-before group-hover:befpre:opacity-100 overflow-hidden">
  ...
  return (
    href ?
      ...
      :
      <div>
        <button className={clsx("relative p-0.5 g5 rounded-2xl shadow-500 group", containerClassName)} onClick={() => {}}>
          <Inner />
        </button>
      </div>
  );
